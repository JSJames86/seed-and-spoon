...
import { recipes as legacyRecipes, getRecipeBySlug as getLegacyRecipeBySlug } from '@/data/recipes';
...
function mergeRecipe(dbRecipe, ingredients) {
  const legacy = getLegacyRecipeBySlug(dbRecipe.slug);
...
/**
 * Fetches a single published recipe by slug, merged with data/recipes.js
 * fallback content. Falls back to the static catalog entry for that slug
 * if the DB is unreachable. Returns null if no recipe exists for the slug.
...
export const getRecipeBySlug = cache(async function getRecipeBySlug(slug) {
  ...
  if (error || !data) {
    return getLegacyRecipeBySlug(slug) ?? null;
  }

  return mergeRecipe(data, (data.recipe_ingredients ?? []).map((i) => i.raw_text).filter(Boolean));
});
