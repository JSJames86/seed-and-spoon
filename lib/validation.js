...
/**
 * Admin - Volunteer schema (for CRUD operations)
 */
export const adminVolunteerSchema = z.object({
  fullName: z.string().min(2, 'Name must be at least 2 characters').max(100),
  email: z.string().email('Invalid email address'),
  phone: z.string().regex(/^[\d\s\-\(\)\+]+$/, 'Invalid phone number').optional().nullable(),
  preferredContact: z.enum(['email', 'text', 'phone']).default('email'),
  roles: z.array(z.string()).min(1, 'Please select at least one volunteer role'),
  availability: z.string().min(10, 'Please describe availability').max(500),
  status: z.enum(['pending', 'active', 'inactive', 'archived']).default('pending'),
  resume: z.string().url('Resume must be a valid URL').optional().nullable(),
  linkedin: z.string().url('LinkedIn must be a valid URL').optional().nullable(),
  accessibilityNotes: z.string().max(500).optional().nullable(),
...
/**
 * Admin - Donor schema (for CRUD operations)
 */
export const adminDonorSchema = z.object({
  fullName: z.string().min(2, 'Name must be at least 2 characters').max(100),
  email: z.string().email('Invalid email address'),
  phone: z.string().regex(/^[\d\s\-\(\)\+]+$/, 'Invalid phone number').optional().nullable(),
  address: z.object({
    ...
    zip: z.string().regex(/^\d{5}$/, 'ZIP code must be 5 digits').optional().nullable(),
  }).optional().nullable(),
  donorType: z.enum(['individual', 'business', 'foundation', 'other']).default('individual'),
  preferredContact: z.enum(['email', 'phone', 'mail']).default('email'),
  status: z.enum(['active', 'inactive', 'lapsed']).default('active'),
  tags: z.array(z.string()).default([]),
  notes: z.string().max(2000).optional().nullable(),
  totalDonated: z.number().nonnegative().default(0),
...
/**
 * Admin - Donation schema (for CRUD operations)
 */
export const adminDonationSchema = z.object({
  donorId: z.string().uuid('Invalid donor ID'),
  amount: z.number().positive('Amount must be positive'),
  currency: z.string().length(3).toLowerCase().default('usd'),
  donationType: z.enum(['one_time', 'monthly', 'in_kind']).default('one_time'),
  paymentMethod: z.enum(['credit_card', 'bank_transfer', 'check', 'cash', 'other']).default('credit_card'),
  ...
  category: z.enum(['general', 'program', 'capital', 'restricted']).default('general'),
  campaign: z.string().max(100).optional().nullable(),
  notes: z.string().max(1000).optional().nullable(),
  isAnonymous: z.boolean().default(false),
  taxReceiptSent: z.boolean().default(false),
  thankYouSent: z.boolean().default(false),
});

/**
 * Admin - Event schema (for CRUD operations)
 */
export const adminEventSchema = z.object({
  title: z.string().min(3, 'Title must be at least 3 characters').max(200),
  description: z.string().min(10, 'Description must be at least 10 characters').max(2000),
  eventType: z.enum(['volunteer', 'fundraiser', 'distribution', 'community', 'training', 'other']).default('volunteer'),
  ...
  location: z.object({
    ...
    virtualLink: z.string().url('Invalid virtual link').optional().nullable(),
  }).optional().nullable(),
  capacity: z.number().int().positive().optional().nullable(),
  spotsRemaining: z.number().int().nonnegative().optional().nullable(),
  ...
  contactPhone: z.string().regex(/^[\d\s\-\(\)\+]+$/, 'Invalid phone number').optional().nullable(),
  tags: z.array(z.string()).default([]),
