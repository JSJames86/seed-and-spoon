...
async function geocodeZip(zip) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.error(`[googlePlacesClient] Zippopotam geocode for zip ${zip} returned ${res.status}`);
    }
  } catch (err) {
    console.error(`[googlePlacesClient] Zippopotam geocode failed for zip ${zip}:`, err.message);
  }
  ...
  if (apiKey && coords) {
    try {
      ...
      if (res.ok) {
        ...
        console.error(`[googlePlacesClient] Places nearby search for zip ${zip} returned ${res.status}: ${await res.text().catch(() => '')}`);
      }
    } catch (err) {
      console.error(`[googlePlacesClient] Places nearby search failed for zip ${zip}:`, err.message);
    }
  }
