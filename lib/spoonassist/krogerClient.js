...
async function getToken() {
  ...
  try {
    ...
    console.error('[krogerClient] token fetch failed:', err.message);
    return null;
  }

  if (!res.ok) {
    console.error(`[krogerClient] token request returned ${res.status}: ${await res.text().catch(() => '')}`);
    return null;
  }

  let json;
  try { json = await res.json(); } catch (err) {
    console.error('[krogerClient] token response JSON parse failed:', err.message);
    return null;
  }
...
export async function getKrogerStoresNearZip(zip, limit = 3) {
  ...
  try {
    ...
    console.error(`[krogerClient] locations fetch failed for zip ${zip}:`, err.message);
    return [];
  }

  if (!res.ok) {
    console.error(`[krogerClient] locations request for zip ${zip} returned ${res.status}: ${await res.text().catch(() => '')}`);
    return [];
  }

  let json;
  try { json = await res.json(); } catch (err) {
    console.error(`[krogerClient] locations response JSON parse failed for zip ${zip}:`, err.message);
    return [];
  }
  ...
  const stores = (json?.data ?? [])
    .map(loc => {
      ...
      const chain    = KROGER_CHAIN_MAP[chainKey];
      // Unmapped chains are typically sandbox/placeholder location records
      // (e.g. "Harris Teeter - - Unused Spoke") — never render raw Kroger
      // fields to users, so skip rather than fall back to loc.name/loc.chain.
      if (!chain) {
        console.error(`[krogerClient] skipping unmapped chain "${loc.chain}" for locationId ${loc.locationId}`);
        return null;
      }
      return {
...
export async function fetchKrogerPrice(ingredientName, qty, unit, locationId = null, zip = null) {
  ...
  try {
    ...
    console.error(`[krogerClient] products fetch failed for "${ingredientName}":`, err.message);
    return null;
  }

  if (!res.ok) {
    console.error(`[krogerClient] products request for "${ingredientName}" returned ${res.status}: ${await res.text().catch(() => '')}`);
    return null;
  }

  let json;
  try { json = await res.json(); } catch (err) {
    console.error(`[krogerClient] products response JSON parse failed for "${ingredientName}":`, err.message);
    return null;
  }
