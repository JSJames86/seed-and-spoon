...
//
// Logged once per cold start so the resolved base URL (and whether it came
// from the override) is visible in deployment logs — Vercel sets
// NODE_ENV=production for both Production and Preview builds, so the
// NODE_ENV-based default alone doesn't tell you which API a deploy is
// hitting.
...
export function getInstacartBaseUrl() {
  ...
  if (!loggedBaseUrl) {
    loggedBaseUrl = true;
    const source = override ? 'INSTACART_API_BASE_URL override' : `NODE_ENV=${process.env.NODE_ENV} default`;
    console.log(`[instacart] base URL: ${baseUrl} (${source})`);
  }
