...
export async function resolveIngredientPrice(ingredientName, quantity, unit, storeMultiplier = 1.0, supabaseClient = null, locationId = null, zipCode = null) {
  ...
  if (locationId || (process.env.KROGER_CLIENT_ID && process.env.KROGER_CLIENT_SECRET)) {
    try {
      ...
      console.error(`[priceEngine] Kroger price lookup failed for "${normalized}":`, err.message);
      /* fall through */
    }
  }
  ...
  if (supabaseClient) {
    try {
      ...
      console.error(`[priceEngine] ingredient_mappings lookup failed for "${normalized}":`, err.message);
      /* fall through */
    }
  }
  ...
  if (supabaseClient) {
    try {
      ...
      console.error(`[priceEngine] price_points lookup failed for "${normalized}":`, err.message);
      /* fall through */
    }
  }
...
export async function getStoresByZip(zip) {
  ...
  if (verified !== null) supplements = verified;
