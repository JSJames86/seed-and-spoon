...
export const STORE_REGISTRY = {
  ...
  'target-union':      { id: 'target-union',       name: 'Target',       priceMultiplier: 1.05, address: '2521 US-22, Union, NJ 07083' },
  // Middlesex County (088-089xx)
...
// ZIP prefix → store IDs served in that area
const ZIP_PREFIX_STORES = {
  '070': ['shoprite-essex', 'aldi-essex', 'stopshop-essex', 'walmart-essex'],
  '071': ['shoprite-essex', 'aldi-essex', 'stopshop-essex'],
  '072': ['shoprite-union', 'stopshop-union', 'wholefoods-union', 'target-union'],
  '073': ['shoprite-union', 'aldi-essex', 'stopshop-essex', 'walmart-essex'],
  '074': ['shoprite-hudson', 'traderjoes-hudson', 'wholefoods-hudson', 'stopshop-hudson'],
  '075': ['shoprite-bergen', 'traderjoes-bergen', 'wholefoods-bergen', 'stopshop-bergen'],
  '076': ['shoprite-bergen', 'traderjoes-bergen', 'stopshop-bergen', 'walmart-essex'],
  '088': ['shoprite-middlesex', 'wegmans-middlesex', 'walmart-middlesex', 'aldi-middlesex'],
  '089': ['shoprite-middlesex', 'wegmans-middlesex', 'walmart-middlesex', 'aldi-middlesex'],
};
...
// ---------------------------------------------------------------------------
// Instacart live price (stub — activated when INSTACART_API_KEY is set)
// ---------------------------------------------------------------------------

async function fetchInstacartPrice(normalized, qty, unit) {
  // Real implementation would call Instacart Developer Platform API here.
  // Returns null until key is provided so we fall through to next tier.
  return null;
}
...
export async function resolveIngredientPrice(ingredientName, quantity, unit, storeMultiplier = 1.0, supabaseClient = null) {
  ...
  if (process.env.INSTACART_API_KEY) {
    try {
      const livePrice = await fetchInstacartPrice(normalized, quantity, unit);
      if (livePrice !== null) {
...
export function getStoresByZip(zip) {
  ...
  const ids = ZIP_PREFIX_STORES[prefix] || ZIP_PREFIX_STORES['070'];
...
export async function calculateRecipeCost(ingredients, stores, supabaseClient = null) {
