...
import {
  ...
  type CommunityPartnerPayload,
  type CommunityPartnerRow,
} from '@/lib/types/communityPartner'
...
export async function POST(request: Request) {
  let body: Partial<CommunityPartnerPayload>
  try {
  ...
  if (!body.organizationType || !(ORGANIZATION_TYPES as readonly string[]).includes(body.organizationType)) {
  ...
  const interests = Array.isArray(body.partnershipInterests)
    ? body.partnershipInterests.filter((i) => (PARTNERSHIP_INTERESTS as readonly string[]).includes(i))
    : []
