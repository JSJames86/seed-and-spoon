 ...
 * CampaignsListClient
 *
 * Client-side fetch/filter interactivity for the campaigns list page.
 * Moved out of app/campaigns/page.jsx so that file can be a server
 * component and export static `metadata`.
 *
 * @component
 */
export default function CampaignsListClient() {
  ...
  const fetchCampaigns = async () => {
    try {
      ...
      if (json.ok) {
        ...
        setFilteredCampaigns(json.data);
      }
    } catch (error) {
  ...
  const otherCampaigns = filteredCampaigns.filter(c => !c.is_featured);
    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-wrap gap-3 mb-8">
          {['all', 'general', 'emergency', 'project', 'annual'].map(type => (
            <button
              key={type}
            ...
            >
              {type === 'all' ? 'All Campaigns' : type.charAt(0).toUpperCase() + type.slice(1)}
            </button>
          ))}
        </div>
          ...
          <>
            {featuredCampaigns.length > 0 && (
              <div className="mb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {featuredCampaigns.map(c => <CampaignCard key={c.id} campaign={c} featured />)}
                </div>
              </div>
            )}
            {otherCampaigns.length > 0 ? (
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {otherCampaigns.map(c => <CampaignCard key={c.id} campaign={c} />)}
                </div>
              </div>
            ) : (
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-3">
          ...
          {campaign.is_matching && (
            <span className="bg-orange-100 text-orange-800 text-xs font-bold px-2 py-1 rounded-full ml-2">Matching!</span>
          )}
        </div>
        <p className="body-sm text-gray-600 mb-4 line-clamp-3">{campaign.description}</p>

        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-bold text-charcoal">{formatCurrency(campaign.amount_raised)}</span>
            <span className="text-sm text-gray-600">of {formatCurrency(campaign.goal_amount)}</span>
          </div>
        ...
        {campaign.impact_metric_label && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
            <p className="text-xs text-green-800 font-medium">{campaign.impact_metric_label}</p>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 mb-4 pt-4 border-t border-gray-100">
          ...
          {['facebook', 'twitter', 'linkedin'].map(p => (
            <button key={p} onClick={() => handleShare(p)} className="text-gray-400 hover:text-blue-600 transition-colors" aria-label={`Share on ${p}`}>
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                {p === 'facebook' && <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>}
                {p === 'twitter' && <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>}
                {p === 'linkedin' && <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>}
              </svg>
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          <Link href={`/campaigns/${campaign.slug}`} className="flex-1 text-center bg-gray-100 text-charcoal py-2 px-4 rounded-lg hover:bg-gray-200 font-bold transition-all text-sm">
            Learn More
          </Link>
          <Link href={`/donate?campaign=${campaign.slug}`} className="flex-1 text-center bg-primary-soil text-white py-2 px-4 rounded-lg hover:bg-gradient-green font-bold transition-all text-sm shadow-md">
            Donate
          </Link>
        </div>
      </div>
