 ...
 * Displays a list of volunteers with filtering and pagination
 */
...
/**
 * @param {Object} props
 * @param {Function} props.onViewDetails - Callback when viewing volunteer details
 * @param {Function} props.onEdit - Callback when editing a volunteer
 */
export default function VolunteerList({ onViewDetails, onEdit }) {
  ...
  const { volunteers, loading, error, refetch, pagination } = useVolunteers(filters);
                ...
                {volunteers.map((volunteer) => (
                  <tr key={volunteer.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                      {volunteer.roles?.slice(0, 2).join(', ')}
                      {volunteer.roles?.length > 2 && ` +${volunteer.roles.length - 2}`}
                    </td>
          ...
          {pagination && (
            <div className="bg-gray-50 dark:bg-gray-700 px-6 py-3 flex items-center justify-between border-t border-gray-200 dark:border-gray-600">
              <div className="text-sm text-gray-700 dark:text-gray-300">
                Showing {pagination.offset + 1} to {Math.min(pagination.offset + itemsPerPage, pagination.total)} of {pagination.total} volunteers
              </div>
