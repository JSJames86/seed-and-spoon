...
// ============================================================================
// CONFIGURATION
// Enable logo rotation by setting ENABLE_LOGO_ROTATION to true
// ============================================================================
const ENABLE_LOGO_ROTATION = false; // Set to true to enable rotating logos

const ROTATING_LOGOS = [
  '/assets/logo/seed-and-spoon-logo-full.png',
  '/assets/logo/seed-and-spoon-logo-full-compact.png',
];
...
export default function Header() {
  ...
  // Optional logo rotation
  useEffect(() => {
    if (!ENABLE_LOGO_ROTATION) return;

    const interval = setInterval(() => {
      setLogoIndex((prev) => (prev + 1) % ROTATING_LOGOS.length);
  ...
  const currentLogo = ENABLE_LOGO_ROTATION
    ? ROTATING_LOGOS[logoIndex]
    : (scrolled
        ? '/assets/logo/seed-and-spoon-logo-full-compact.png'
        : '/assets/logo/seed-and-spoon-logo-full.png');
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between">
          {/* Logo */}
          <Link href="/" className="py-2" aria-label="Seed & Spoon NJ - Home">
            <img
              src={currentLogo}
              alt="Seed & Spoon NJ"
              ...
              style={{
                opacity: ENABLE_LOGO_ROTATION ? 1 : undefined,
              }}
            />
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <nav className="hidden md:flex items-center gap-6 lg:gap-8" role="navigation" aria-label="Main navigation">
              ...
              <Link href="/causes" className="text-gray-700 hover:text-green-600 font-medium transition-colors">
                Causes
              </Link>
              <Link href="/volunteer" className="text-gray-700 hover:text-green-600 font-medium transition-colors">
                Volunteer
              </Link>
              <Link
                href="/donate"
                className="bg-green-600 text-white px-6 py-2 rounded-full hover:bg-green-700 font-semibold transition-colors shadow-md hover:shadow-lg"
              >
                Donate
              </Link>
            </nav>
            ...
            <button
              ...
              className="md:hidden text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 rounded-lg p-1"
      ...
      <div
        id="mobile-menu"
        className={`fixed right-0 top-0 bottom-0 z-50 w-80 max-w-[85vw] bg-white/95 backdrop-blur-md shadow-2xl md:hidden transform transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          ...
          <button
            onClick={closeMobileMenu}
            className="text-gray-700 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 rounded-lg p-1"
        ...
        <nav className="flex flex-col gap-2 p-4" role="navigation" aria-label="Mobile navigation">
          <Link
            href="/"
            onClick={closeMobileMenu}
            className="text-gray-700 text-lg font-medium py-3 px-4 rounded-lg hover:bg-green-50 hover:text-green-600 transition-colors"
          >
            Home
          </Link>
          <Link
          ...
          <Link
            href="/donate"
            onClick={closeMobileMenu}
            className="bg-green-600 text-white text-lg font-semibold py-3 px-6 rounded-full hover:bg-green-700 transition-colors shadow-md hover:shadow-lg text-center mt-2"
          >
            Donate
          </Link>
